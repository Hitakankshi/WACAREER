...
function createBubbles(canvas: HTMLCanvasElement, config: any = {}): CreateBubblesReturn {
...
const Bubbles: React.FC<BubblesProps> = ({ className = '', followCursor = true, ...props }) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const spheresInstanceRef = useRef<CreateBubblesReturn | null>(null);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    spheresInstanceRef.current = createBubbles(canvas, {
      followCursor,
      ...props
    });
